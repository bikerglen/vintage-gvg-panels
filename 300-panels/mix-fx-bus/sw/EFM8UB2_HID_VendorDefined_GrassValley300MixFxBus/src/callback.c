...
extern volatile uint8_t needSend;
extern volatile uint8_t sendData[2];

extern volatile uint8_t updateNeeded;
extern volatile uint8_t updateRow;
extern volatile uint8_t updateCol;
	...
	if (isIdleTimerExpired() == true)
	{
	    if (USBD_EpIsBusy(EP1IN) == false) {
	    	if (needSend) {
	    		...
	    		txBuffer[1] = sendData[0]; // row
	    		txBuffer[2] = sendData[1]; // key
				USBD_Write (EP1IN, txBuffer, 3, false);
				needSend = false;
		    }
	    }
	}
                             ...
                             uint16_t remaining)
{
	...
	if (epAddr == EP1OUT) {
		if (rxBuffer[0] == 0x02) {
			updateNeeded = true;
			updateRow = rxBuffer[1];
			updateCol = rxBuffer[2];
		}
		USBD_Read (EP1OUT, rxBuffer, 16, true);
	}
